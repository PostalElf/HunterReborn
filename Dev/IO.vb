...
<DebuggerStepThrough()> Public Class IO
    ...
    Public Shared Function SimpleFilegetAll(ByVal pathname As String) As List(Of String)
        ...
        End Try
    End Function
    Public Shared Function CSVFileget(ByVal pathname As String, ByVal divider As Char, Optional ByVal skipFirstRow As Boolean = False) As List(Of String())
        Dim total As New List(Of String())
        Using sr As New StreamReader(pathname)
            If skipFirstRow = True Then sr.ReadLine()
            While sr.Peek <> -1
                Dim raw As String = sr.ReadLine.Trim
                If raw = "" Then Continue While
                Dim rsplit As String() = raw.Split(divider)
                total.Add(rsplit)
            End While
        End Using
        Return total
